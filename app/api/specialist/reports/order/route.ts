...
export async function POST(request: NextRequest) {
    const { dentista, paciente, examenes, observaciones } = await request.json();

    if (!paciente || !paciente.nombre || !paciente.apellido || !examenes || !observaciones) {
    ...
    // Issue
    cursorY -= 40;
    drawCentered("ORDEN MÉDICA", 12, cursorY, rgb(0, 0, 0), bold);
    cursorY -= 40;
    ...
    // Examinations table
    drawCentered("Exámenes indicados", 12, cursorY, rgb(0, 0, 0), bold);
    cursorY -= 30;

    // Table header
    const colNombreX = marginX;

    page.drawText("Examen", { x: colNombreX, y: cursorY, size: 11, font: bold });
    cursorY -= 20;

    // Table body
    examenes.forEach((t: { nombre: string, descripcion: string, precio: number }) => {
