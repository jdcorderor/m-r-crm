...
export async function POST(request: NextRequest) {
    const { dentista, paciente, tratamientos, observaciones } = await request.json();

    if (!paciente || !paciente.nombre || !paciente.apellido || !tratamientos || !observaciones) {
    ...
    // Issue
    cursorY -= 40;
    drawCentered("PRESUPUESTO", 12, cursorY, rgb(0, 0, 0), bold);
    cursorY -= 40;
    ...
    // Treatments table
    drawCentered("Tratamientos indicados", 12, cursorY, rgb(0, 0, 0), bold);
    cursorY -= 30;

    // Table header
    const colNombreX = marginX;
    const colPrecioX = pageWidth - marginX - 100;

    page.drawText("Tratamiento", { x: colNombreX, y: cursorY, size: 11, font: bold });
    page.drawText("Precio", { x: colPrecioX, y: cursorY, size: 11, font: bold });
    cursorY -= 20;

    // Table body
    tratamientos.forEach((t: { nombre: string, descripcion: string, precio: number }) => {
        page.drawText(t.nombre, { x: colNombreX, y: cursorY, size: 10, font: font });
        page.drawText(`USD ${t.precio.toFixed(2)}`, { x: colPrecioX, y: cursorY, size: 10, font: font });
        cursorY -= 18;
    });
    ...
    const totalLabelWidth = bold.widthOfTextAtSize(totalLabel, fontSize);

    page.drawText(totalLabel, { x: marginX, y: cursorY, size: 11, font: bold, color: rgb(0, 0, 0) });

    page.drawText(`USD ${tratamientos.reduce((acc: number, tratamiento:  { nombre: string, descripcion: string, precio: number }) => { return acc + tratamiento.precio }, 0).toFixed(2)}`, { x: marginX + totalLabelWidth + 6, y: cursorY, size: 11, font: bold });
    cursorY -= 45;

    // Observations
    drawCentered("Observaciones", 12, cursorY, rgb(0, 0, 0), bold);
    cursorY -= 30;
