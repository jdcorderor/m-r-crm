...
import { TreatmentForm } from "@/app/types/treatment"
...
export default function Page() {
    ...
    // State variable for treatment registration form fields
    const [treatment, setTreatment] = useState<TreatmentForm>({ nombre: "", descripcion: "", duracion: 0, precio: 0, caracteristicas: [], imagen_url: "", activo: false });

    // State variable for characteristics input
    const [characteristic, setCharacteristic] = useState("");

    // Treatment registration handler
    const handleTreatmentRegistration = async (event: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                setTreatment({ nombre: "", descripcion: "", duracion: 0, precio: 0, caracteristicas: [], imagen_url: "", activo: false });
    ...
    return (
        <section>
            ...
            {/* Treatment registration section */}
            <main className="flex justify-center items-center min-h-[80vh]">
                <div className="bg-white w-full max-w-5xl p-10">
                    <div>
                        <span className="block text-2xl text-gray-800 font-semibold mb-8 text-center">Registro de Servicio/Tratamiento</span>
                        <form className="space-y-8" onSubmit={ handleTreatmentRegistration }>
                            <div>
                                    ...
                                    {(treatment.caracteristicas && treatment.caracteristicas.length > 0) && (
                                        <div className="border-y border-gray-200 my-3 py-4">
                                            <table className="min-w-full border border-gray-300 rounded-md overflow-hidden">
                                                <thead className="bg-gray-100 text-left text-sm font-medium text-gray-800">
                                                    <tr>
                                                        <th className="px-4 py-2">Característica del servicio/tratamiento</th>
                                                        <th className="px-4 py-2 text-center">Acciones</th>
                                                    </tr>
                                                </thead>
                                                <tbody className="text-sm text-gray-800">
                                                    {treatment.caracteristicas.map((c, index) => (
