...
export default function Page() {
  ...
  // State variable for current reservation data
  const [reservation, setReservation] = useState<{ id: number; odontologo_id: number; paciente: string; cedula: string; fecha_nacimiento: string; email: string; telefono: string; genero: string; fecha: string | null; fin_tentativo: string | null; motivo: string } | null>(null);
  ...
  useEffect(() => {
    const handleDataLoad = async () => {
        ...
        const response = await fetch(`/api/assistant/reservations/${id}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
            },
  ...
  const handleSubmit = () => {
    if (reservation?.odontologo_id !== null && reservation?.fecha && reservation?.fin_tentativo) {
      setShowModal(true);
      return;
    }
  }
  ...
  const handleReservationConfirmation = async () => {
    try {
      const response = await fetch("/api/assistant/reservations/confirm", {
        ...
        body: JSON.stringify({ id: reservation?.id, fecha: reservation?.fecha, fin_tentativo: reservation?.fin_tentativo, odontologo_id: reservation?.odontologo_id, motivo: reservation?.motivo }),
        credentials: "include",
      });
        ...
        <div> 
          ...
          <div className="flex flex-col w-full max-w-3xl bg-gray-50 border border-gray-200 rounded-2xl p-16 mx-auto my-16"> 
            <div className="flex flex-col w-full max-w-3xl text-sm gap-4">
              <span className="block text-gray-800 text-2xl text-center font-semibold">Confirmación de Citas</span>

              <hr className="border-gray-200"/>

              <div className="flex flex-col w-full max-w-[600px] mx-auto py-2 gap-1">
                <span><b>Paciente:</b> {reservation?.paciente}</span>
                <span><b>Cédula de identidad:</b> {reservation?.cedula}</span>
                <span><b>Correo electrónico:</b> {reservation?.email}</span>
                <span><b>Teléfono:</b> {reservation?.telefono}</span>
              </div>
              ...
              <div className="flex flex-col overflow-hidden py-6 gap-4">
                <Calendar onHandleChange={ handleDatetimeSelect } data={{ id: id?.toString(), fecha: reservation?.fecha, fin_tentativo: reservation?.fin_tentativo }} confirmedDates={ confirmedReservations } dentistID={ (reservation?.odontologo_id != null) ? (reservation.odontologo_id).toString() : null }/> 
              </div>
          ...
          {showModal && (
            <div className="fixed inset-0 flex items-center justify-center z-50 bg-transparent bg-opacity-30 backdrop-blur-sm ">
              <div className="bg-white rounded-lg shadow-lg p-8 max-w-sm w-full">
                ...
                <span className="block text-center text-sm mb-8">Al confirmar la cita, se enviará una notificación al correo electrónico del paciente: <strong>{reservation?.email}</strong>.</span>
