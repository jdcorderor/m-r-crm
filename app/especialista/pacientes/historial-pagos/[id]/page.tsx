...
export default function Page() {
    ...
    useEffect(() => {
        const fetchRecord = async () => {
            ...
            try {
                const response = await fetch(`/api/specialist/payments-record/${id}/${user.username}`, {
                    method: "GET",
                    headers: {
                        "Content-Type": "application/json",
                    },
    ...
    const handlePaymentRegistration = async (consultation: number) => {
        try {
            ...
            router.push(`/especialista/pacientes/historial-pagos/${id}`);
        } catch (error) {
            console.error("Error al obtener pacientes:", error);
        }
    };
    ...
    function calculateBalance() {
        return consultationPayments.reduce((acc, c) => { 
            const totalAmount = c.monto_total ? c.monto_total : 0;
                                ...
                                <div>
                                    ...
                                    {consultationPayments.length === 0 && (
                                        <div className="flex flex-col w-full justify-center gap-8 pt-8 text-center text-sm text-gray-600">
                                            <p>El paciente <b>{record?.nombre} {record?.apellido}</b> no tiene transacciones registradas</p>
                                        </div>
                                    )}
                                </div>
                            </div>
