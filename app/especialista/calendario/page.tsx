...
export default function Page() {
    ...
    useEffect(() => {
        const fetchMonthAppointments = async (date: Date): Promise<void> => {
            ...
            try {
                const response = await fetch(`/api/specialist/calendar/${user.username}`, {
                    method: "GET",
                    headers: {
                        "Content-Type": "application/json",
                    },
    ...
    const hasAppointments = (day: number): boolean => {
        const dayKey = `${selectedDate.getFullYear()}-${(selectedDate.getMonth() + 1).toString().padStart(2, "0")}-${day.toString().padStart(2, "0")}`;
        ...
        <section>
            ...
            {!isLoading && (
                <div>
                    ...
                    <div className="flex flex-col max-w-5xl mx-auto my-12">
                        <main className="w-full bg-gray-50 border border-gray-200 rounded-xl p-12">
                                        ...
                                        <ul className="space-y-4">
                                            {appointments.map((appointment) => (
                                                <li key={appointment.id} className="bg-white p-4 border border-gray-200 rounded-lg shadow-sm">
                                                    ...
                                                    <button className="flex text-xs text-white font-medium bg-blue-600 opacity-80 hover:opacity-90 rounded-full px-4 py-1 mt-4 ml-auto" onClick={ () => { if (appointment.date > new Date().toISOString()) { router.push(`/especialista/pacientes/${appointment.record_id}`) } } }>Ver paciente</button>
                                                </li>
                                            ))}
                                        </ul>
                                    )}
